...
@withWeapp({
  ...
  data: {
    ...
    _img_top: Taro.getSystemInfoSync().windowHeight / 2, //图片上边距
    _img_left: Taro.getSystemInfoSync().windowWidth / 2, //图片左边距
    watch: {
      //监听截取框宽高变化
      width(value, that) {
  ...
  attached() {
    this.data.info = Taro.getSystemInfoSync()
    //启用数据监听
    this._watcher()
  ...
  methods: {
    ...
    imgReset() {
      this.setData({
        scale: 1,
        angle: 0,
        _img_top: Taro.getSystemInfoSync().windowHeight / 2,
        _img_left: Taro.getSystemInfoSync().windowWidth / 2
      })
    },
    /**
     * 加载（更换）图片
    ...
    _initCanvas() {
      //初始化canvas
      if (!this.data.ctx) {
        this.data.ctx = Taro.createCanvasContext('image-cropper', this)
      }
    },
    /**
     * 根据开发者设置的图片目标尺寸计算实际尺寸
...
class _C extends Taro.Component {
  ...
  render() {
    ...
    return (
      <View className="image-cropper" onTouchMove={this._preventTouchMove}>
        <View
          className="main"
        ...
        >
          <View className="content">
            <View
              className={
                'content_top bg_gray ' + (_flag_bright ? '' : 'bg_black')
              }
              style={
                'height:' +
                cut_top +
                'px;transition-property:' +
                (_cut_animation ? '' : 'background')
              }
            ></View>
            <View className="content_middle" style={'height:' + height + 'px;'}>
              <View
                className={
                  'content_middle_left bg_gray ' +
                  (_flag_bright ? '' : 'bg_black')
                }
                style={
                  'width:' +
                  cut_left +
              ...
              ></View>
              <View
                className="content_middle_middle"
                style={
                  'width:' +
                  width +
                  'px;height:' +
                  height +
                  'px;transition-duration: .3s;transition-property:' +
                  (_cut_animation ? '' : 'background') +
                  ';'
                }
              >
              ...
              <View
                className={
                  'content_middle_right bg_gray ' +
                  (_flag_bright ? '' : 'bg_black')
                }
                style={
                  'transition-property:' + (_cut_animation ? '' : 'background')
                }
              ></View>
            </View>
            <View
              className={
                'content_bottom bg_gray ' + (_flag_bright ? '' : 'bg_black')
              }
              style={
                'transition-property:' + (_cut_animation ? '' : 'background')
              }
          ...
          <Image
            ...
            style={
              'width:' +
              (img_width ? img_width + 'px' : 'auto') +
              ';height:' +
              (img_height ? img_height + 'px' : 'auto') +
              ';transform:translate3d(' +
              (_img_left - img_width / 2) +
              'px,' +
        ...
        <Canvas
          ...
          style={
            'width:' +
            _canvas_width * export_scale +
            'px;height:' +
            _canvas_height * export_scale +
            'px;left:' +
            canvas_left +
            'px;top:' +
            canvas_top +
